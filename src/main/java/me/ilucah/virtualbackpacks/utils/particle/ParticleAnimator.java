...
public class ParticleAnimator {
    ...
    private static BiConsumer<Player, ParticleEffect> reefer() {
    ...
    private static BiConsumer<Player, ParticleEffect> rings() {
        return ((player, particle) -> new BukkitRunnable() {
            double alpha = 0;
            float t = 0;
    ...
    private static BiConsumer<Player, ParticleEffect> stomp() {
        return ((player, particle) -> new BukkitRunnable() {
            double t = Math.PI / 4;
            Location loc = player.getLocation();
    ...
    private static BiConsumer<Player, ParticleEffect> star() {
    ...
    private static BiConsumer<Player, ParticleEffect> frostLord() {
        return ((player, particle) -> new BukkitRunnable() {
            double t = 0;
            double pi = Math.PI;
    ...
    private static BiConsumer<Player, ParticleEffect> disco() {
        return ((player, particle) -> {
            double radius = 3;
            int particles = 30;
    ...
    private static BiConsumer<Player, ParticleEffect> cone() {
        return (player, particle) -> {
            float lengthGrow = .05f;
            double angularVelocity = Math.PI / 16;
            int particles = 10;
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    for (int x = 0; x < particles; x++) {
                        double angle = iteration * angularVelocity + rotation;
                        float radius = iteration * radiusGrow;
                        float length = iteration * lengthGrow;
                        Vector v = new Vector(Math.cos(angle) * radius, length, Math.sin(angle) * radius);
                        rotateAroundAxisX(v, (location.getPitch() + 90) * (180f / Math.PI));
                        rotateAroundAxisY(v, -location.getYaw() * (180f / Math.PI));
